...
Router (AGENT 1, cheap model)               ← sets routing_flag BEFORE the analyzer
   ...
   ↓
inject-dr.js (bundler)                      ← regenerates the 6-DR-file bundle; analyzer Reads it (no auto-inject)
   ↓
...
THE DR MARKETING KNOWLEDGE FILES are NOT auto-injected — there is no such mechanism, and you run
as a subagent where settings hooks do not fire. As your FIRST step you MUST
`Read prompts/_generated/section-analyzer-dr-context.md` — the bundle of the six files below,
generated by `tools/hooks/inject-dr.js`. If it is missing or stale, regenerate it with
`node tools/hooks/inject-dr.js` then Read it. Do NOT proceed without it. The files supply the
belief taxonomy rationale, persuasion element definitions, and vocabulary for classifying
execution_type, proof_tier, and move. Use them as classification rubrics. The bundled files are:
