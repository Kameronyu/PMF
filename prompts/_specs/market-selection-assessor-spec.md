...
- competitive_axis (function-capability-price | visual-statement | community-openness) + a page-quoted basis
...
- **2.2 Differentiating axis.** Check for a differentiator on mechanism AND on avatar. Neither = KILL (true me-too — entering as the nth identical voice teaches nothing). The named axis that passes IS the UM. **Use competitive_axis here:** read whether the cell competes on function-capability-price, visual-statement, or community-openness, and judge whether the product's UM lands on a DIFFERENTIATING axis vs that field. (This is the transparency-as-UM read for hardware-novelty products: if the cell competes on function/price, a visual-statement UM is a real differentiator; if the cell already competes on visual-statement, the UM must out-do them on it or it's me-too.) NOTE: a real differentiator at 2.2 is also what can JUSTIFY a price premium flagged at 2.1c — connect the two.
