...
- **#1 Asset classification → ENGINEERING (operator-confirmed).** The `09-relevance-bucket` / `10-role-classify` / `11-comprehend-video` *logic* is engineering/perceptual (what's in the pixels). Its deterministic bricks are now hardened + smoke-proven this run — `asset-fetch` (local), `asset-map-rank`→`asset-emit` (incl. the section-table path fix), `product-video-assemble`, `validate-asset-record` (all `working` in `REGISTRY.json`, smokes `engine/contracts/h6-asset-*.sh`). The classifier *logic* is reused via the build spec `prompts/_specs/image-classifier-brick.md` (§2). Any marketing-flavored copy in the `marketing-lens/09/10/11` prompt *instances* is re-authored at R5 — the instances are NOT carried across the firewall. (The perceptual video-frame py helpers probe_video/frame-grab/sample_montage build records upstream and are exercised only in a live comprehend-video run — R7.)
