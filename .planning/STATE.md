...
- Phase 21 **DONE (2026-06-25)**: Engine wiring hardening — H0–H5 executed. Summary: `.planning/phases/21-engine-wiring-hardening-fix-arduview-retrieval-failures-and-/21-SUMMARY.md`. 9 atomic commits (`a2209b4`→`cc73b76`); REGISTRY health flipped untested→working for 10 capabilities (validators→enums.json, validate-analyzer fires, funnel-score/clean/store, embeddings-RAG, meta-ad-fetch graphql extraction, cred-seam). H5 E2E spine passes on `runs/_fixture/` (`engine/contracts/h5-e2e.sh`). 11 working incl. **Trends FIXED** (`f2c6555` — the 429 was a cold-consent gate, not the IP; consent-warm + multiline XHR intercept works from this AWS IP). reddit-extract hardened to headed (`6dc6a30`) but a clean dump needs a residential IP / OAuth (datacenter IP rate-limits reddit). belief_kind backfill deferred (judgment job, quarantine-bound). Next = REBUILD-ROADMAP.md R3 quarantine (arduview-v2 appended). *(Added 2026-06-25): Engine wiring hardening — fix Arduview retrieval/wiring failures + smoke each engine capability green. **Source of truth = `engine/contracts/HARDENING.md` + `engine/contracts/RETRIEVAL-FAILURES-arduview.md`, NOT this roadmap** (frozen/untrusted pending the marketing rebuild). Engine-only; no marketing. Wiring was reorg'd `tools/`→`engine/` (`ad5b510`); recovery tag `m1-arduview-retro`. `.planning/codebase/` map is STALE (pre-reorg) — use `engine/contracts/REGISTRY.md` instead.)*
