...
- **PHASE 6 PENDING (final).** SMOKE-01..05 + deferred SMOKE-05 (pipeline-completeness / canonical-order assertion: controller walks pipeline.yaml as-is, a DROPPED step exits 0 → DoD smoke must catch it as dangling). `run all --space=smoke` already completes clean end-to-end (achieved early in Phase 4); Phase 6 formalizes the DoD harness + zero-orphan/dangling assertion, then milestone lifecycle (`gsd-audit-milestone` → `gsd-complete-milestone`). Remaining content-job deferrals (NOT this milestone): real field-level seam schemas (CONTENT-02), real prompt bodies, prompt-division 1-vs-N, Phase B.
...
## To resume
Phases 1–5 are DONE (6 gates green: store-smoke, controller-smoke, h6-all [deterministic], manifest-smoke, stub-smoke, validate-smoke). Read this file → dispatch **Phase 6 (Smoke Run & DoD — SMOKE-01..05 + deferred SMOKE-05)** per the conductor recipe, then the milestone lifecycle (`gsd-audit-milestone` → `gsd-complete-milestone`). This is the FINAL phase. Memory file `project_pmf_shell_milestone.md` also has the high-level state.
